...
class Korkmaz_woo_sales_contract_Admin
{
	...
	public function vergi_no_dairesi($order)
	{
		...
		if (get_post_meta($order->get_id(), '_billing_company', true) == null) {
			echo '<p><strong>' . __('Tc kimlik no', 'korkmaz_contract') . ':</strong> ' . esc_html(get_post_meta($order->get_id(), '_shipping_tc', true)) . '</p>';
		} else {
			$vergiDaire = get_post_meta($order->get_id(), '_billing_vergi_dairesi', true);
			$vergiNum = get_post_meta($order->get_id(), '_billing_vergi_nosu', true);
	...
	public function butonlari_goster_admin($order)
	{
		...
		$birinci_sozlesme = get_post_meta($order->get_id(), 'birinci_sozlesme_dosya', true);
		$ikinci_sozlesme = get_post_meta($order->get_id(), 'ikinci_sozlesme_dosya', true);
		$upload_dir = home_url();
